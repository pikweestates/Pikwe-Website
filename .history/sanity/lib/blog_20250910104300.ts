...
export async function getNextBlogPosts(
  slug: string,
  limit = 3
): Promise<BlogShort[]> {
  if (!slug) return [];
  ...
  const afterPosts: BlogShort[] = await client.fetch(afterQuery, {
    publishedAt,
  });
  ...
  const fillPosts: BlogShort[] = await client.fetch(topFillQuery, {
    excludeIds,
  });
...
// @/sanity/lib/blog.ts (add this function)
