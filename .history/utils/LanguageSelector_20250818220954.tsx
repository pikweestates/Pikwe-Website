...
export default function LanguageSelector({status, setActiveSide, setLocalState}: {status: string, setActiveSide: React.Dispatch<React.SetStateAction<boolean>>, setLocalState: React.Dispatch<React.SetStateAction<string>>}) {
  ...
  const handleChange = async (newLocale: string) => {
    ...
    // Check if the current page is 'projets' or 'projects'
    if (newPathname.includes("/confidentialite") || newPathname.includes("/privacy")) {
      // Adjust the pathname based on the new locale
      newPathname =
        newLocale === "fr"
  ...
  return (
    <div className={styles.ls__container} ref={containerRef} onClick={() => setActiveSelector((v) => !v)}>
      <div className={styles.ls__wrapper}>
        <span className={`${styles.selected} ${status==="sidebar" ? styles.side : ""}`}>{currentLocale.toUpperCase()}</span>
        <span className={`${styles.icon} ${activeSelector ? styles.active__icon : ""} `}>
          <Icon icon="fa:angle-down" className={`${status==="sidebar" ? styles.side : ""}`} />
        </span>
      </div>
      <div className={`${styles.selector} ${activeSelector ? styles.active__selector : ""}`}>
        <div
