...
export interface BlogPost {
  ...
  excerpten: string,
  excerptfr: string,
  body: TypedObject | Type
}

const BlogHeader = ({ animationFinished, blogpost }: { animationFinished: boolean, blogpost: BlogPost }) => {
  //Translations
  const { t, i18n } = useTranslation();
  const currentlocale = i18n.language;

  // const categoriess = ["Market Insights", "Legal Guidance"];
  const categories = blogpost.categories.map((category: Category) => ({
  ...
  const sectionData = {
    ...
    h2: t("Blog:quick"),
    text: currentlocale==="en" ? blogpost.excerpten : blogpost.excerptfr,
  };
      ...
      <div className={`container ${styles.bh__container}`}>
        <div className={styles.bh__top}>
          <Copy animationFinished={animationFinished}>
            <span className={styles.bh__date}>{currentlocale==="en" ? formatDate(blogpost.publishedAt) : formatDate(blogpost.publishedAt, "fr-FR")}</span>
          </Copy>
          <Copy animationFinished={animationFinished}>
            <h2 className={styles.bh__h2}>
              {currentlocale==="en" ? blogpost.titleen : blogpost.titlefr}
            </h2>
          </Copy>
            ...
            {categories.map((data, i) => (
              <Copy key={i} animationFinished={animationFinished}>
                <span className={styles.category}>
                  {currentlocale==="en" ? data.nameen : data.namefr}
                </span>
              </Copy>
            ))}
          </div>
          ...
          <div className={styles.bh__image} ref={container}>
            <motion.div className={styles.bhi__wrapper} style={{ y }}>
              <ImagePlaceholder src={
                urlFor(blogpost.image).width(1920).url() || urlFor(blogpost.image).url()
              }
