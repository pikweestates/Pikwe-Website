...
const FilterContainer = ({ filterData }: { filterData: FilterInterface }) => {
  ...
  //Helpful during clearing all filters
  const currentParam = searchParams.get(filterData.filterName.slug);

  // 1. State for the selected filter name
  const [selectedName, setSelectedName] = useState(() =>  currentParam ? currentParam : filterData.filterName.fn);

  useEffect(() => {
    // whenever the URL param changes, reset the label
    if (currentParam) {
      // find matching filter name
      const match = filterData.filters.find(f => f.slug === currentParam);
      setSelectedName(match ? match.name : filterData.filterName.fn);
    } else {
      setSelectedName(filterData.filterName.fn);
    }
  }, [currentParam, filterData]);
  ...
  return (
    ...
    >
      <div className={`${styles.fc__top} ${selectedName !== filterData.filterName.fn ? styles.activeFilter : ""}`}>
        ...
        <div className={`${styles.fc__icon} ${activeModal ? styles.active__icon : ""}`}>
      ...
      <div
        ...
        } ${["size", "surface"].includes(filterData.filterName.slug) && styles.fc__bottom2}`}
      >
