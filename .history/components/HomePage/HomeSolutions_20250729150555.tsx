...
const HomeSolutions = ({lenis, currentLocale}: {lenis: Lenis | null, currentLocale: string}) => {
  //Translations
        ...
        <div className={styles.sol__content}>
          <div className={styles.sc__top}>
            {solutionsContent.map((data, i) => (
              <div className={`${styles.accordion}`} key={i} onClick={() => setActiveAccordion(i)}>
                <div className={styles.acc__head}>
                  <div className={`${styles.acc__iconwrap} ${activeAccordion===i ? styles.active__svg : ""}`}>
                    <Icon icon="ix:plus" />
                  </div>
                  <div className={styles.ah__right}>
                    <h2 className={`${styles.acc__h2} ${activeAccordion===i ? styles.active__h2 : ""}`}>{data.name}</h2>
                  </div>
                </div>
                <div className={`${styles.acc__body} ${activeAccordion===i ? styles.active__acc : ""}`}>
