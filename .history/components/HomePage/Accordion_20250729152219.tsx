...
const Accordion = ({lenis, data, index, activeAccordion, setActiveAccordion}: {lenis: Lenis | null, index: number, data: DataInterface, activeAccordion: number, setActiveAccordion: React.Dispatch<React.SetStateAction<number>>; }) => {
      ...
      <div className={styles.acc__head}>
        <div
          className={`${styles.acc__iconwrap} ${
            activeAccordion === index ? styles.active__svg : ""
          }`}
        >
          <Icon icon="ix:plus" />
        </div>
        <div className={styles.ah__right}>
          <h2
            className={`${styles.acc__h2} ${
              activeAccordion === index ? styles.active__h2 : ""
            }`}
          >
            {data.name}
          </h2>
