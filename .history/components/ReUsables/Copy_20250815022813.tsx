  ...
  const startAnimation = () => {
    ...
    elements.forEach((element) => {
      ...
      const split = new SplitText(element, {
        type: "lines",
        mask: "lines",
        linesClass: "split-line++", // Safe class name
      });
    ...
    // Set masked state *before* showing
    gsap.set(lines.current, { yPercent: 100 });
    setHidden(false); // reveal container now
    ...
    if (animateOnScroll) {
      gsap.to(lines.current, {
        ...
        scrollTrigger: {
          ...
          start: "top 75%",
          once: true,
          immediateRender: false, // avoid re-applying mid-paint
        },
      });
