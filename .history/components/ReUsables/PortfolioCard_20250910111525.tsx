...
const PortfolioCard = ({ data }: { data: PortfolioInterface }) => {
  ...
  return (
    <Link href={data.link} className={styles.portfolio__container} prefetch={true}>
      <div className={styles.pc__top}>
        <div className={styles.pc__image}>
          <ImagePlaceholder src={urlFor(data.image).width(1920).url() || urlFor(data.image).url()} alt={data.image.alt ? data.image.alt : data.name} />
        </div>
