...
const SquareButton = ({status, content}: {status: string, content: contentInterface}) => {
  ...
  if (status === "light"){
    ...
    return (
      <Link href={content.} className={styles.button__container}>
        <span>{t("Navigation:buttontext")}</span>
      </Link>
    )
  } else if (status === "ivory"){
    return (
      <Link href={content.} className={styles.button__container2}>
        <span>{t("Navigation:buttontext")}</span>
      </Link>
    )
  } else {
    return (
      <Link href={content.} className={styles.button__container3}>
        <span>{t("Navigation:buttontext")}</span>
      </Link>
    )
  }
