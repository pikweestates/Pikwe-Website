...
const GallerySection = ({
  ...
  const galleryContent = gallery.map((content) => ({
    image: content.image,
    text: currentlocale==="en" ? content.descriptionen : content.descriptionfr,
  }));
              ...
              >
                <div className={styles.gbm__top}>
                  <div className={styles.gb__main}>
                    <ImagePlaceholder src={urlFor(data.image).width(1920).url() || urlFor(data.image).url()} alt={data.text} />
                  </div>
                  <div className={styles.gb__overlay}>
                    ...
                    <div className={styles.gb__bottom}>
                      <div className={styles.thumbnail}>
                        <ImagePlaceholder
                          src={urlFor(data.image).width(1920).url() || urlFor(data.image).url()}
                          alt={data.text}
                        />
                      </div>
      ...
      <AnimatePresence mode="wait">
        {activeModal && (
          <motion.div
            ...
            animate={activeModal? "enter" : "closed"}
            exit="closed"
            initial="initial"
          ...
          >
            <div className={styles.main__image}>
              <ImagePlaceholder
                src={urlFor(galleryContent[activeImage].image).width(1920).url() || urlFor(galleryContent[activeImage].image).url()}
                alt={galleryContent[activeImage].text}
              />
              <div
