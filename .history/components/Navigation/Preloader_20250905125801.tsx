...
const Preloader = ({
  ...
  useEffect(() => {
    const determinePageName = () => {
      if (pathname === "/en" || pathname === "/fr") {
        ...
        setPageName(pagename || t("HomePage:error"));
      } else {
  ...
  useEffect(() => {
    if (localState !== "Translating State") {
      setLanguageReady(true); // not in translating mode -> no wait
      return;
    }

    // if already in the target language, show immediately
    setLanguageReady(false);
    const onLang = () => setLanguageReady(true);
    i18n.on("languageChanged", onLang);

    // Safety fallback: don't block forever (optional)
    const to = setTimeout(() => setLanguageReady(true), 1000);
  ...
  return (
    <div className={styles.preloader} ref={preloaderRef}>
      {localState === "Translating State" && languageReady && (
